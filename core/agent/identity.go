...
func (a *Agent) generateIdentity(guidance string, userID, agentID uuid.UUID, model string) error {
	...
	err := llm.GenerateTypedJSON(a.context.Context, a.client, "Generate a character as JSON data. "+guidance, model, userID, agentID, a.options.character.ToJSONSchema(), &a.options.character)
